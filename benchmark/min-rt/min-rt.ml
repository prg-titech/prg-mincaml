...
(* ラジアン *)
(*MINCAML*)
let rec rad x = x *. 0.017453293 (*NOMINCAML let rad x = x *. (0.017453293)*) in
(**** 環境データの読み込み ****)
(*MINCAML*)
let rec read_environ _ =
  (*NOMINCAML let read_environ _ =*)
  (*if dbg.(0) then Format.printf "reading environment data.@.";*)
  (* スクリーン中心の座標 *)
  screen.(0) <- read_float ();
  screen.(1) <- read_float ();
  screen.(2) <- read_float ();
...
let rec read_nth_object n =
  ...
  (* if dbg.(0) then Format.printf "object #%d.@." n;*)
  ...
  then (
    ...
    (* ; if dbg.(0) then Format.eprintf "OBJ #%d: %d (%f %f %f) (%f %f %f) [%f
       %f %f]@." n form abc.(0) abc.(1) abc.(2) xyz.(0) xyz.(0) xyz.(0)
...
(* solver : オブジェクト (の index) と、ベクトル L, P を受けとり、 直線 Lt + P と、オブジェクトとの交点を求める。
   交点がない場合は 0 を、交点がある場合はそれ以外を返す。 この返り値は nvector で交点の法線ベクトルを求める際に必要。 (直方体の場合)
...
let rec solver index l p =
  ...
  if m_shape = 1
  then solver_rect m l
  else if m_shape = 2
  then solver_surface m l
...
let rec is_rect_outside m =
  ...
  if if abs_float isoutside_q.(0) < o_param_a m
     then
       if abs_float isoutside_q.(1) < o_param_b m
       then if abs_float isoutside_q.(2) < o_param_c m then true else false
       else false
     else false
  then o_isinvert m
...
let rec is_outside m =
  ...
  let m_shape = o_form m in
  if m_shape = 1
  then is_rect_outside m
  else if m_shape = 2
  then is_plane_outside m
...
let rec check_all_inside ofs iand =
  ...
  if head = -1
  then true
  else if is_outside objects.(head)
  then false
...
let rec shadow_check_and_group iand_ofs and_group p =
  ...
  then false
    ...
    (* if crashed_object.(0) = obj then shadow_check_and_group (iand_ofs + 1)
       and_group p else *)
    ...
    then (
      ...
      then true
        ...
        (* 次のオブジェクトから候補点を探す *))
    else if (* 交点がない場合: 極性が正(内側が真)の場合、 *)
            (* AND ネットの共通部分はその内部に含まれるため、*)
            (* 交点はないことは自明。探索を打ち切る。 *)
            o_isinvert objects.(obj)
...
let rec shadow_check_one_or_matrix ofs or_matrix p =
  ...
  then false (* OR行列の終了マーク *)
  else if range_primitive = 99
  then
    (* range primitive はない *)
...
let rec solve_each_element iand_ofs and_group =
  ...
  then ()
    ...
    else if (* 交点がなく、しかもその物体は内側が真ならこれ以上交点はない *)
            o_isinvert objects.(iobj)
    then ()
...
let rec get_nvector m p =
  ...
  let m_shape = o_form m in
  if m_shape = 1
  then get_nvector_rect ()
  else if m_shape = 2
  then get_nvector_plane m
  else if (* 2次曲面 or 錐体 *)
          o_isrot m <> 0
  then get_nvector_second_rot m p
...
let rec utexture m p =
  ...
  then (
    ...
    let w7 =
      if abs_float w1 < 1.0e-4
      then 15.0 (* atan +infty = pi/2 *)
      else (
...
let rec scan_line scany =
  ...
  if scany < size.(0)
  then (
    (* if dbg.(0) then ( print_string "scanning y = "; print_int scany;
       print_string "\n"; ) else ();*)
    scan_sscany.(0)
