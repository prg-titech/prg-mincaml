...
and g' dest cont regenv = function
  ...
  | (Nop | Li _ | SetL _ | Comment _ | Restore _ | FLi _) as exp ->
    Ans exp, regenv
  ...
  | Stw (x, y, z') ->
    ( Ans (Stw (find x Type.Int regenv, find y Type.Int regenv, find' z' regenv))
    , regenv )
  ...
  | Stfd (x, y, z') ->
    ( Ans
        (Stfd (find x Type.Float regenv, find y Type.Int regenv, find' z' regenv))
    , regenv )
  ...
  | CallCls (x, ys, zs) as exp ->
    if List.length ys > Array.length regs - 2
       || List.length zs > Array.length fregs - 1
    then
      failwith
