...
let rec g env = function
  ...
  | Closure.MakeCls ((x, t), { Closure.entry = l; Closure.actual_fv = ys }, e2)
    ...
    let offset, store_fv =
      expand
        ...
        (fun y _ offset store_fv -> seq (Stw (y, x, C offset), store_fv))
    in
    Let
      ( (x, t)
      , Mr reg_hp
      , Let
          ...
          , let z = Id.genid "l" in
            Let ((z, Type.Int), SetL l, seq (Stw (z, x, C 0), store_fv)) ) )
  | Closure.AppCls (x, ys) ->
  ...
  | Closure.Tuple xs ->
    ...
    let offset, store =
      expand
        ...
        (fun x _ offset store -> seq (Stw (x, y, C offset), store))
    in
    Let
  ...
  | Closure.Get (x, y) ->
    ...
    | Type.Array Type.Float ->
      Let ((offset, Type.Int), Slw (y, C 3), Ans (Lfd (x, V offset)))
    | Type.Array _ ->
      Let ((offset, Type.Int), Slw (y, C 2), Ans (Lwz (x, V offset)))
    | _ -> assert false)
  | Closure.Put (x, y, z) ->
    let offset = Id.genid "o" in
    (match M.find x env with
    | Type.Array Type.Unit -> Ans Nop
    | Type.Array Type.Float ->
      Let ((offset, Type.Int), Slw (y, C 3), Ans (Stfd (z, x, V offset)))
    | Type.Array _ ->
      Let ((offset, Type.Int), Slw (y, C 2), Ans (Stw (z, x, V offset)))
    | _ -> assert false)
...
let h
  ...
  let offset, load =
    expand
      ...
      (fun z t offset load -> Let ((z, t), Lwz (x, C offset), load))
  in
  match t with
  | Type.Fun (_, t2) ->
