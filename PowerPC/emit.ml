...
let reg r = if is_reg r then String.sub r 1 (String.length r - 1) else r
...
and g' oc = function
  ...
  | ( Tail
    , ((Li _ | SetL _ | Mr _ | Neg _ | Add _ | Sub _ | Slw _ | Lwz _) as exp) )
    ->
    g' oc (NonTail regs.(0), exp);
    Printf.fprintf oc "\tblr\n"
  | ( Tail
    , ((FLi _ | FMr _ | FNeg _ | FAdd _ | FSub _ | FMul _ | FDiv _ | Lfd _) as
      exp) ) ->
  ...
  | NonTail a, CallCls (x, ys, zs) ->
    ...
    then Printf.fprintf oc "\tmr\t%s, %s\n" (reg a) (reg regs.(0))
    else if List.mem a allfregs && a <> fregs.(0)
    then Printf.fprintf oc "\tfmr\t%s, %s\n" (reg a) (reg fregs.(0));
